...
<input type="radio" name="answer[<?php echo $qk;?>][]"  onClick="showan(this.value);"  id="answer_value<?php echo $qk.'-'.$i;?>" value="<?php echo $option['oid'];?>"   <?php if(in_array($option['oid'],$save_ans)){ echo 'checked'; } ?>  >
		 ...
		 </td><td> <?php echo str_replace('../../../',base_url(),str_replace('../../../../',base_url(),$option['q_option']));?>
		</td></tr></table>
...
<?php echo str_replace('../../../',base_url(),str_replace('../../../../',base_url(),$option['q_option']));?></td></tr>
		</table>
		 </div>
	...
	foreach($options as $ok => $option){
				if($option['qid']==$question['qid']){
				echo str_replace('../../../',base_url(),str_replace('../../../../',base_url(),$option['q_option'])).' ';
				}
				}
				?>	
		</div>	 
		...
		<table>  <?php
		 foreach($options as $ok => $option){
				if($option['qid']==$question['qid']){
					...
					$match_2[]=$option['q_option_match'];
			?>
			<tr><td><?php echo str_replace('../../../',base_url(),str_replace('../../../../',base_url(),$option['q_option']));?></td><td><?php echo $option['q_option_match'];?></td></tr>
