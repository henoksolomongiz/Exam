...
class Payment_gateway_2 extends CI_Controller {
	...
	public function subscribe($gid='0',$uid='0',$quid='0')
	{
...
if($quid==0){
foreach($gid as $h => $v){
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Payumoney'.$_POST,'0');
}
}else{
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Payumoney'.$_POST,$quid);	
}
...
if($isValidChecksum == "TRUE") {
  ...
  // following file need to be included
    ...
    $transactionURL = PAYTM_STATUS_QUERY_URL;
...
if($quid==0){
foreach($gid as $h => $v){
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Paytm',$responseReader,'0');
}
}else{
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Paytm'.$_POST,$quid);	
}
   ...
   echo "<b>Transaction status is not confirmed from paytm server. Contact admin for manual investigation</b>" . "<br/>"; 
}
...
if($quid==0){
foreach($gid as $h => $v){
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'2Checkout',$responseReader,'0');
} 
}else{
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Payumoney'.$_POST,$quid);	
}       
...
if($quid==0){
foreach($gid as $h => $v){
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Paypal',$responseReader,'0');
}
}else{
$this->payment_model_2->activate_group($uid,$v,$amount,$transaction_id,'Payumoney'.$_POST,$quid);	
}  
